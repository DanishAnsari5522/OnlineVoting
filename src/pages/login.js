...
export default function Login({tokenExpired}) {
  ...
  const loginButton = async (e) => {
    ...
    let result = await fetch('http://localhost:5000/v1/user/login', {
      method: 'POST',
      headers: {
    ...
    .catch(err => {
      ...
      // informationText.current.innerText = 'Account is not active, check email for activation or contact the administrator!'
      // else
      // informationText.current.innerText = 'Incorrect email/password!'
