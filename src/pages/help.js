...
export default function HelpTicket({ userId, userType, changePage }) {
    ...
    const fetchHelps = async () => {
        try {
            setLoading(true);
            let APIurl = `/api/v1/help?parent${userType !== 'SUPER-ADMIN' ? `&userId=${userId}` : ''}`
    ...
    if (loading)
        ...
        return (
            <>
                <p>We Are here to help you.please drop a message...</p>
            </>
        )
}